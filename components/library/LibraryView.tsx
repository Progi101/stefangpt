...
// Fix: Define a more specific type for library content to preserve type narrowing.
...
const LibraryView: React.FC<LibraryViewProps> = ({ sessions, onToggleHistory }) => {
  ...
  const handleDownload = async (imageUrl: string, prompt: string) => {
    try {
        ...
        // Fallback for CORS or other issues: open in a new tab
        window.open(imageUrl, '_blank');
    }
  };

  const renderItem = (item: LibraryItem) => {
      // Fix: Destructure item.content to aid TypeScript's type narrowing.
      const { content } = item;
